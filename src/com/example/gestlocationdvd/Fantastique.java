...
public class Fantastique extends Activity {
	
	private SQLiteDatabase db;
	private DatabaseFilm databaseFilm;
	private ListView listFantastique;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.fantastique);
		listFantastique=(ListView) findViewById(R.id.Action);
		...
		//exécuter une requete Select avec comme condition categorie=Fantastique
		...
		if(nbRec>0){
			...
			listFantastique.setAdapter(adapterfilm);
			
		}
		
		listFantastique.setOnItemClickListener(new OnItemClickListener() {
			
			@Override
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				
				Film film = new Film();
				film = (Film) listFantastique.getItemAtPosition(position);
				
				 /*SharedPreferences prefs = getSharedPreferences("Fantastique", MODE_PRIVATE);
				 SharedPreferences.Editor editor = prefs.edit();
				 editor.putInt("Fantastique", id)
				 editor.commit(); // This line is IMPORTANT. If you miss this one its not gonna work!*/
				Bundle test=getIntent().getExtras();
				Intent intent = new Intent(Fantastique.this, VuDetail.class);
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		// Inflate the menu; this adds items to the Fantastique bar if it is present.
...
//		item.setIcon(R.drawable.abc_list_divider_mtrl_alpha);
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		{
		case R.id.item1 : 
			Intent intent = new Intent(Fantastique.this, Accueil.class);
			
			intent.putExtra("id_client", test.getInt("id_client"));
			startActivity(intent);
			break;
		case R.id.item2:
			Intent intent2 = new Intent(Fantastique.this, About.class);
			Bundle test2=getIntent().getExtras();
			intent2.putExtra("id_client", test2.getInt("id_client"));
			startActivity(intent2);
			break;
		case R.id.item3:
			Intent intent3 = new Intent(Fantastique.this, Connexion.class);
			startActivity(intent3);
			break;
		case R.id.item4:
			Intent intent4 = new Intent(Fantastique.this, Searchable.class);
